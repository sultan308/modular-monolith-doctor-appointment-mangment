...
impl DoctorAppointmentsManagementController {
    pub async fn get_all_doctor_appointments(&self, doctor_id: ObjectId, from: Option<DateTime<Utc>>, to: Option<DateTime<Utc>>) -> Result<Vec<ResponseAppointment>> {
        ...
        let appointments = self.appointments_services.get_all_doctor_appointments_in_range(doctor_id,from,to).await?;
    ...
    pub async fn cancel_appointment(&self, appointment_id: ObjectId) -> Result<ResponseAppointment> {
        let canceled_appointment = self.appointments_services.cancel_appointment(appointment_id).await?;
        Ok(ResponseAppointment::from(canceled_appointment))
    }
    pub async fn complete_appointment(&self, appointment_id: ObjectId) -> Result<ResponseAppointment> {
        let completed_appointment = self.appointments_services.complete_appointment(appointment_id).await?;
